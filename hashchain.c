...
/*
    print_list
        displays the linked-list as [key, value]
        pairs in sequence
    param:
        record_t* head (a pointer to the head of the list)
    return:
        void
*/
void print_list(record_t* head)
{
    //while a record in the given list exists,
    //display each key-value record in sequence
    while(head)
    {   
